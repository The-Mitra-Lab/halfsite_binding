...
def add_cbf1_score(cf):
	gal4_cutoff = 5
	cf = add_pwm_info(cf,'gal4', gal4_cutoff)
	score_list = []
	max_pos_list = []
	for idx,row in cf.iterrows():
		maxscore = 0
		maxpos = 0
		for j,score in enumerate(row["gal4 score"]):
			if score > maxscore:
				maxscore = score
				maxpos = row["gal4 pos"][j]
		score_list.append(maxscore)
		max_pos_list.append(maxpos)
	cf["gal4 Max Score"] = score_list
	cf["gal4 Max Pos"] = max_pos_list
	return cf
